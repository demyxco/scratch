...
git remote set-url origin https://${DEMYX_GITHUB_TOKEN}@github.com/demyxco/scratch.git
git add .; git commit -m "Travis Build $TRAVIS_BUILD_NUMBER"; git push origin HEAD:master || exit 0
