...
[[ ! -d "$DEMYX"/wp-content/mu-plugins ]] && /bin/mkdir -p "$DEMYX"/wp-content/mu-plugins
/bin/cp "$DEMYX_CONFIG"/bs.php "$DEMYX"/wp-content/mu-plugins

# Configure xdebug
if [[ ! -d "$DEMYX"/.vscode ]]; then
    /bin/mkdir -p "$DEMYX"/.vscode
    /bin/mv "$DEMYX_CONFIG"/launch.json "$DEMYX"/.vscode
fi

# Set wp-config.php to debug mode
/bin/sed -i "s|'WP_DEBUG', false|'WP_DEBUG', true|g" "$DEMYX_WP_CONFIG"
